...
object PlanLimits {
    fun maxReplyRules(tier: PlanTier): Int = when (tier) {
        PlanTier.FREE -> 10
        PlanTier.PRO -> 250
        PlanTier.MAX -> 2000
    }

    fun maxMessagesPerMonth(tier: PlanTier): Int = when (tier) {
        PlanTier.FREE -> 100
        PlanTier.PRO -> 5000
        PlanTier.MAX -> 50000
    }

    fun maxAiRepliesPerMonth(tier: PlanTier): Int = when (tier) {
        PlanTier.FREE -> 20
        PlanTier.PRO -> 1000
        PlanTier.MAX -> 10000
    }

    fun maxBroadcastRecipients(tier: PlanTier): Int = when (tier) {
        PlanTier.FREE -> 50
        PlanTier.PRO -> Int.MAX_VALUE
        PlanTier.MAX -> Int.MAX_VALUE
    }

    fun showsBroadcastBranding(tier: PlanTier): Boolean = tier == PlanTier.FREE
}
